...
def export_json(conn: sqlite3.Connection):
    ...
    # JSON軽量化：スナップショットを直近40件に制限
    MAX_SNAPSHOTS = 40
    if len(snapshots) > MAX_SNAPSHOTS:
        snapshots_trimmed = snapshots[-MAX_SNAPSHOTS:]
        trim_offset = len(snapshots) - MAX_SNAPSHOTS
    else:
        snapshots_trimmed = snapshots
        trim_offset = 0
