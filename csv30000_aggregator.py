...
def compute_item_movers(conn, snapshots):
    ...
    def attach_history(items, period_start_dt):
        """TOP入りした各アイテムにperiod_start_dt〜最新の価格推移を付与する"""
        period_snaps = [s for s in snapshots if period_start_dt <= s <= last_dt]
        if len(period_snaps) > 12:
            # 多すぎる場合は等間隔で12点に間引き（先頭・末尾は必ず残す）
            step = (len(period_snaps) - 1) / 11.0
            idxs = sorted(set(round(i * step) for i in range(12)))
            period_snaps = [period_snaps[i] for i in idxs]
        for it in items:
            rows = conn.execute("""
                SELECT snapshot_dt, best_price FROM item_snapshot
                WHERE jan = ? AND best_price > 0 ORDER BY snapshot_dt
            """, (it["jan"],)).fetchall()
            price_map = {r[0]: r[1] for r in rows}
            it["history"] = [price_map.get(s) for s in period_snaps]
        return period_snaps
    ...
    for key, prev_dt in period_dts.items():
        ...
        # TOP入りアイテムにのみ週内推移を付与（サイズ抑制のため）
        used_snaps = attach_history(up, prev_dt)
        attach_history(dn, prev_dt)
        result["comparison"][key] = {
            "compared_to": prev_dt,
            "snapshots": [s[5:16] for s in used_snaps],  # "MM-DD HH:MM"
            "up": up,
            "down": dn,
        }
