...
def compute_item_movers(conn, snapshots):
    ...
    # トレカ判定用キーワード（名前に含まれたらカード扱い）
    import re as _re
    TCG_RE = _re.compile(
        r'PSA|BGS|ARS|CGC|ポケモンカード|ポケカ|遊戯王|ワンピースカード|デュエマ|'
        r'\bSR\b|\bSAR\b|\bUR\b|\bHR\b|\bRRR\b|\bSSR\b|\bCSR\b|\bCHR\b|\d{3}/\d{3}|'
        r'プロモ|未開封BOX|シュリンク|スターバース|VSTARユニバース|シャイニートレジャー',
        _re.IGNORECASE
    )

    TOP_N = 300  # 各方向・各期間の最大件数（容量と実用性のバランス）
    ...
    for key, prev_dt in period_dts.items():
        ...
        for jan, prev_price in prev_map.items():
            ...
            name = curr[1] if curr[1] else ""
            is_tcg = 1 if TCG_RE.search(name) else 0
            diffs.append({
                "jan":       jan,
                "name":      name[:25],
                "category":  curr[2],
        ...
        # 上昇TOP500・下降TOP500（金額順）— historyは持たせない（軽量化）
        up = sorted([d for d in diffs if d["change"] > 0], key=lambda x: -x["change"])[:TOP_N]
        dn = sorted([d for d in diffs if d["change"] < 0], key=lambda x: x["change"])[:TOP_N]
