...
# GAS連携（デプロイ後のURLを設定）
GAS_URL     = "https://script.google.com/macros/s/AKfycbw63qoENNKZTxhKERh8xhUwTqtJtv3CA12to_IOXL-smSWPUtNUq6TC3se56cg6m9S-ig/exec"
...
def compute_item_movers(conn, snapshots):
    ...
    for key, prev_dt in period_dts.items():
        ...
        # 上昇TOP50・下降TOP50（金額順）
        up = sorted([d for d in diffs if d["change"] > 0], key=lambda x: -x["change"])[:50]
        dn = sorted([d for d in diffs if d["change"] < 0], key=lambda x: x["change"])[:50]
...
def fetch_watchlist_with_trends(conn, snapshots):
    ...
    for ent in entries:
        ...
        prices = [dt_map[s][1] if s in dt_map else None for s in snapshots]
        amazons = [dt_map[s][2] if s in dt_map else None for s in snapshots]
        latest = rows[-1] if rows else None
        result.append({
            "jan": jan,
            "name": (latest[4] if latest else "") or ent.get("name", ""),
            "category": latest[5] if latest else "",
            "current_price": latest[1] if latest else None,
            "current_store": latest[3] if latest else "",
...
def push_to_gas(payload: dict):
    ...
    if not GAS_URL:
        return
    import requests as _req
    body = {"key": GAS_ACCESS_KEY, "data": payload}
    try:
        ...
        if result.get("error"):
            ...
            logger.info("GASへ送信しました（%d bytes）", result.get("size", 0))
    except Exception as exc:
